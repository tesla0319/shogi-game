...
**SHOGI-4 駒打ち基盤（4a〜4h）+ USI指し手変換（4i）完了 / 次は SHOGI-4 の対局進行本体（CLI対局・終局判定）**
...
- SHOGI-4（対局進行）: 駒打ち基盤（4a〜4h）+ USI指し手変換（4i）完了 — 持ち駒（Hand）・駒打ち（Move.drop）・駒打ち候補生成・二歩/行き所のない駒/打ち歩詰めの除外・合法手への統合・駒取り時の持ち駒加算。USI指し手表記と Move の相互変換（`move_from_usi` / `move_to_usi`。通常手・成り手・駒打ちに対応、形式変換のみで合法性は判定しない）。対局進行本体（CLI対局ループ・終局判定）は未着手
